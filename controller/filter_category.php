<?php
	...
	// echo"</pre>";
				...
				$sql="select ca_name from category where ca_id='$cid'";
				$result=$obj->conn->query($sql) or die($obj->conn->error);
				$ans=$result->fetch_array(MYSQL_NUM);

				echo "<h2>".$ans[0]."</h2> <hr/>";

		$sql="select p_id,p_name,p_mrp,round((p_mrp)-(p_mrp*p_dis/100)) as discount,p_imgpath from product where p_caid='$cid' order by p_id desc";
